...
public interface ProductRepository
    ...
    @Query(
        "SELECT p FROM Product p WHERE p.quantity > 0 AND (p.state.id = 2 OR p.state.id = 4) AND (p.name LIKE :value OR p.description LIKE :value)"
    )
    ...
    @Query(
        "SELECT p FROM Product p WHERE p.quantity > 0 AND p.category = :category AND (p.state.id = 2 OR p.state.id = 4)"
    )
    ...
    @Query(
        "SELECT p FROM Product p WHERE p.quantity > 0 AND p.store = :store AND (p.state.id = 2 OR p.state.id = 4)  ORDER BY p.createdAt DESC"
    )
    ...
    @Query(
        "SELECT p FROM Product p WHERE p.quantity > 0 AND (p.state.id = 2 OR p.state.id = 4) AND (:categoryId IS NULL OR p.category.id = :categoryId) AND (:storeId IS NULL OR p.store.id = :storeId) AND (:minPrice IS NULL OR p.price >= :minPrice) AND (:maxPrice IS NULL OR p.price <= :maxPrice) ORDER BY p.createdAt DESC"
    )
